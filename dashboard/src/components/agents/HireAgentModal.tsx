...
const FALLBACK_ARCHETYPES: AgentArchetype[] = [
  ...
  { name: 'Hermes CEO', role: 'ceo', capabilities: ['strategic-planning', 'task-decomposition', 'tool-calling', 'autonomous-reasoning', 'pipeline-orchestration', 'budget-governance', 'workforce-delegation', 'memory-graph-management'], constraints: ['must verify task completion before declaring success', 'must isolate code edits in git worktrees', 'must log all actions to audit trail', 'must balance workload across workforce agents', 'must respect budget limits'], system_prompt: 'You are Hermes, the Chief Executive Officer and Principal System Orchestrator of NVLabsCompany. Powered by Nous Research Hermes 3. You have full operational authority over all agents, tasks, pipelines, and workflows. Execute tool calls to delegate, coordinate, and verify work across the organization.', tools_allowed: ['task-create', 'task-delegate', 'pipeline-run', 'agent-wake', 'agent-pause', 'memory-store', 'plaza-broadcast', 'budget-check', 'git-worktree', 'code-analysis'], interaction_style: 'directive', description: 'Hermes 3-powered CEO with full orchestration authority. Manages the entire platform on demand: decomposes goals, delegates to workforce, monitors budgets, and ensures quality through autonomous tool calling.' },
...
export function HireAgentModal({ isOpen, onClose, onSuccess }: HireAgentModalProps) {
  ...
  const handleTemplateSelect = (archetype: AgentArchetype) => {
    ...
    if (archetype.name.toLowerCase().includes('hermes') || archetype.role === 'ceo') {
      setProvider('hermes');
      setModel('hermes3:8b');
    }
