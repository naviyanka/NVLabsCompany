...
const INITIAL_PROPOSALS: EvolutionProposal[] = [
  {
    id: 'evo-3041',
    title: 'Enforce Structured JSON Verification Checksums Before Emission',
    agent_id: 'Dwight (QA Lead)',
    category: 'Formatting Constraint',
    score_delta: 14.8,
    status: 'pending',
    original_prompt: 'Prioritize speed over verbose reasoning logs. Execute QA assertions directly.',
    mutated_prompt: 'Enforce concise structured JSON output with explicit verification checksums before final emission. Reject unvalidated schemas.',
    synthetic_evals: {
      passed: 194,
      total: 200,
      latency_delta_ms: -45,
      token_saving_pct: 12.4,
      p_value: 0.0004,
    },
  ...
  {
    id: 'evo-3040',
    title: 'Multi-Tenant Header Validation & Strict Quota Pre-Check',
    agent_id: 'Angela (Budget Auditor)',
    category: 'Safety Guardrail',
    score_delta: 18.2,
    status: 'approved',
    original_prompt: 'Monitor monthly token allocation and calculate agent spending ratios.',
    mutated_prompt: 'Pre-check X-Company-Id isolation headers and halt execution immediately if company spending exceeds 80% monthly threshold quota.',
    synthetic_evals: {
      passed: 199,
      total: 200,
      latency_delta_ms: -28,
      token_saving_pct: 18.6,
      p_value: 0.0001,
    },
  ...
  {
    id: 'evo-3039',
    title: 'GitNexus Call-Graph Impact Analysis Pre-Verification',
    agent_id: 'Jim (PR Reviewer)',
    category: 'Prompt Refinement',
    score_delta: 12.5,
    status: 'approved',
    original_prompt: 'Review pull requests and check for clean TypeScript syntax.',
    mutated_prompt: 'Run impact analysis on symbol call graphs before reviewing pull requests. Highlight high blast radius callers and require zero lint errors.',
    synthetic_evals: {
      passed: 188,
      total: 200,
      latency_delta_ms: -60,
      token_saving_pct: 9.8,
      p_value: 0.0012,
    },
  ...
  {
    id: 'evo-3038',
    title: 'Contradiction Resolution Heuristics Optimization',
    agent_id: 'Kevin (Data Engineer)',
    category: 'Context Optimization',
    score_delta: 8.4,
    status: 'rejected',
    original_prompt: 'Scan memory graph nodes and group matching assertions together.',
    mutated_prompt: 'Calculate Dijkstra shortest path distances and automatically resolve contradicting memory nodes using weighted consensus scoring.',
    synthetic_evals: {
      passed: 162,
      total: 200,
      latency_delta_ms: +35,
      token_saving_pct: 4.2,
      p_value: 0.048,
    },
  ...
  {
    id: 'evo-3037',
    title: 'Zero-Trust Security Audit Tree Traversal Guardrail',
    agent_id: 'Creed (Security Specialist)',
    category: 'Safety Guardrail',
    score_delta: 22.4,
    status: 'pending',
    original_prompt: 'Audit target directories for security vulnerabilities.',
    mutated_prompt: 'Traverse target dependency trees inside isolated sandbox scopes only. Block direct filesystem mutations without explicit checksum confirmation.',
    synthetic_evals: {
      passed: 198,
      total: 200,
      latency_delta_ms: -110,
      token_saving_pct: 21.0,
      p_value: 0.0001,
    },
